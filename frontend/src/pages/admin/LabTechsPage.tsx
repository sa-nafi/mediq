...
import { Microscope, Search, Edit2, Ban, Calendar, Mail } from 'lucide-react';
...
export function LabTechsPage() {
  ...
  const { data: empData, isLoading, error } = useQuery({
    queryKey: ['admin', 'employees', 'lab_tech', page, search],
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin', 'employees', 'lab_tech'] });
      setSelectedEmp(null);
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.message || 'Failed to update lab technician');
    }
  });
  ...
  const deactivateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Lab Technician deactivated successfully');
      queryClient.invalidateQueries({ queryKey: ['admin', 'employees', 'lab_tech'] });
      setDeactivateId(null);
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.message || 'Failed to deactivate lab technician');
    }
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-foreground">Manage Lab Techs</h1>
          <p className="text-muted mt-0.5 text-sm">View, update, or deactivate laboratory technicians.</p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex h-40 items-center justify-center rounded-2xl border border-border-light bg-surface shadow-sm">
          <p className="text-muted text-sm font-medium">Loading lab technicians...</p>
        </div>
      ) : error ? (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-6 flex flex-col items-center justify-center text-red-600 gap-2 shadow-sm">
          ...
          <p className="text-base font-bold">Failed to load lab technicians.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {employees.length > 0 ? (
            employees.map((emp: any) => (
              <div key={emp.employee_id} className="bg-surface rounded-xl p-4 shadow-sm border border-border-light flex flex-col md:flex-row gap-4 md:items-center justify-between hover:border-secondary/30 transition-colors group">
                <div className="flex gap-4 items-center">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-purple-50 border border-purple-100 text-purple-600 group-hover:scale-105 transition-transform">
            ...
            <div className="py-12 text-center bg-surface border border-border-light rounded-2xl shadow-sm">
              <Microscope className="h-12 w-12 text-muted/30 mx-auto mb-4" />
              <h3 className="text-lg font-bold text-foreground">No lab technicians found</h3>
              <p className="text-muted mt-1 text-sm">No lab technicians match your search criteria.</p>
            </div>
          )}
      ...
      {/* Modern Redesigned Edit Lab Tech Dialog */}
      ...
      <ConfirmDialog
        ...
        title="Deactivate Lab Technician"
        description="Are you sure you want to deactivate this lab technician? They will lose access to the system."
        variant="destructive"
        onConfirm={() => {
